...
CubeHandler::CubeHandler(std::shared_ptr<Cube> cube)
...
bool CubeHandler::EdgesOriented() const
{
    std::array<uint32_t, 4> upEdges{ ((m_Cube->m_Up >> 24) & 0x0000000F),
                                     ((m_Cube->m_Up >> 16) & 0x0000000F),
                                     ((m_Cube->m_Up >> 8) & 0x0000000F),
                                     ((m_Cube->m_Up >> 0) & 0x0000000F) };

    std::array<uint32_t, 4> downEdges{ ((m_Cube->m_Down >> 24) & 0x0000000F),
                                       ((m_Cube->m_Down >> 16) & 0x0000000F),
                                       ((m_Cube->m_Down >> 8) & 0x0000000F),
                                       ((m_Cube->m_Down >> 0) & 0x0000000F) };

    std::array<uint32_t, 4> frontEdges{ ((m_Cube->m_Front >> 24) & 0x0000000F),
                                        ((m_Cube->m_Front >> 16) & 0x0000000F),
                                        ((m_Cube->m_Front >> 8) & 0x0000000F),
                                        ((m_Cube->m_Front >> 0) & 0x0000000F) };

    std::array<uint32_t, 4> backEdges{ ((m_Cube->m_Back >> 24) & 0x0000000F),
                                       ((m_Cube->m_Back >> 16) & 0x0000000F),
                                       ((m_Cube->m_Back >> 8) & 0x0000000F),
                                       ((m_Cube->m_Back >> 0) & 0x0000000F) };

    std::array<uint32_t, 4> rightEdges{ ((m_Cube->m_Right >> 24) & 0x0000000F),
                                        ((m_Cube->m_Right >> 16) & 0x0000000F),
                                        ((m_Cube->m_Right >> 8) & 0x0000000F),
                                        ((m_Cube->m_Right >> 0) & 0x0000000F) };

    std::array<uint32_t, 4> leftEdges{ ((m_Cube->m_Left >> 24) & 0x0000000F),
                                       ((m_Cube->m_Left >> 16) & 0x0000000F),
                                       ((m_Cube->m_Left >> 8) & 0x0000000F),
                                       ((m_Cube->m_Left >> 0) & 0x0000000F) };
...
std::vector<std::shared_ptr<CubeHandler>> CubeHandler::GetNeighbors(std::vector<Cube::MOVE> moves)
{
    std::vector<std::shared_ptr<CubeHandler>> neighbors;
    for (const Cube::MOVE& move : moves)
    {
        std::shared_ptr<CubeHandler> handler = std::make_shared<CubeHandler>(
            std::make_shared<Cube>(m_Cube->m_Up, m_Cube->m_Down, m_Cube->m_Front, m_Cube->m_Back,
                                   m_Cube->m_Right, m_Cube->m_Left));
        handler->m_Cube->Twist(move);
        handler->m_Parent = { shared_from_this(), move };
        neighbors.push_back(handler);
    }
    return neighbors;
}

uint32_t CubeHandler::HashCube(uint32_t maxVal) const
{
    uint32_t val = (m_Cube->m_Up + m_Cube->m_Down + m_Cube->m_Front + m_Cube->m_Back +
                    m_Cube->m_Right + m_Cube->m_Left) %
                   maxVal;
    std::mt19937 gen(val);
    std::uniform_int_distribution<> distrib(0, maxVal);
    return distrib(gen);
}